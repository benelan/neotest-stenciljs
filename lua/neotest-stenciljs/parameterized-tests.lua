...
-- Synchronously runs `stencil` in `file_path` directory skipping all tests and returns `stencil` output.
-- Output have all of the test names inside it. It skips all tests by adding
-- extra `--testPathPattern` parameter to stencil command with placeholder string that should never exist.
-- @param file_path string - path to file to search for tests
-- @return table - parsed stencil test results
local function run_stencil_test_discovery(file_path)
  local binary = stencil_util.getStencilTestCommand(file_path)
  ...
  local stencil_json_string = result[2].stdout

  if not stencil_json_string or #stencil_json_string == 0 then
    return nil
  end

  return vim.json.decode(stencil_json_string, { luanil = { object = true } })
end

-- Searches through whole `stencil` command output and returns array of all tests at given `position`.
-- @param stencil_output table
-- @param position number[]
-- @return { keyid: string, name: string }[]
local function get_tests_ids_at_position(stencil_output, position)
  local test_ids_at_position = {}
  for _, testResult in pairs(stencil_output.testResults) do
    ...
    for _, assertionResult in pairs(testResult.assertionResults) do
      ...
      if position[1] <= location.line - 1 and position[3] >= location.line - 1 then
        local keyid = stencil_util.get_test_full_id_from_test_result(testFile, assertionResult)
...
-- First runs `stencil` in `file_path` to get all of the tests in the file. Then it takes all of
...
function M.enrich_positions_with_parameterized_tests(
  ...
  local stencil_test_discovery_output = run_stencil_test_discovery(file_path)

  if stencil_test_discovery_output == nil then
    return
  end
  ...
  for _, value in pairs(parsed_parameterized_tests_positions) do
    ...
    local parameterized_test_results_for_position =
      get_tests_ids_at_position(stencil_test_discovery_output, data.range)
...
-- Replaces all of the stencil parameters (named and unnamed) with `.*` regex pattern.
-- It allows to run all of the parameterized tests in a single run. Idea inspired by Webstorm stencil plugin.
...
function M.replaceTestParametersWithRegex(test_name)
  ...
  for _, parameter in ipairs(STENCIL_PARAMETER_TYPES) do
